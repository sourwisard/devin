...
const CONFIG = {
  ...
  viewerCount: 0,
  // ─── Colors ─────────────────────────────────────────────────────────
  accentColor:  "#3b004e",   // primary accent (name gradient, play btn, seek bar, playlist pill)
...
<script>
  (function () {
    ...
    // Only playlists tagged "showOnCheezit: true" in metadata.json's
    ...
    fetch(METADATA_URL, { cache: 'no-store' })
      ...
      .then((meta) => {
        ...
        const allowedNames = new Set(
          Object.keys(playlistSettings).filter((name) => playlistSettings[name] && playlistSettings[name].showOnCheezit)
        );
